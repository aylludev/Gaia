...
from hermes.forms import PurchasePaymentForm
...
class PurchasePaymentListView(LoginRequiredMixin, ValidatePermissionRequiredMixin, ListView):
    model = Purchase
    template_name = 'purchasepayment/list.html'
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'searchdata':
                ...
                if purchase_id:
                    for i in PurchasePayment.objects.filter(purchase_id=purchase_id):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['list_url'] = reverse_lazy('hermes:purchasepayment_list')
        context['entity'] = 'Cuentas por pagar'
        return context

class PurchasePaymentCreateView(LoginRequiredMixin, ValidatePermissionRequiredMixin, CreateView):
    model = PurchasePayment
    form_class = PurchasePaymentForm
    template_name = 'purchasepayment/create.html'
    success_url = reverse_lazy('hermes:purchasepayment_list')
    permission_required = 'add_purchasepayment'
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'add':
                ...
                form.instance.purchase = self.purchase  # Asignar la compra al formulario
                data = form.save()
            else:
    ...
    def get_context_data(self, **kwargs):
        ...
        context['action'] = 'add'
        context['purchase'] = self.purchase  # Pasar la compra al contexto
        context['pending_balance'] = self.purchase.total - (PurchasePayment.objects.filter(purchase=self.purchase).aggregate(total=Coalesce(Sum('amount'), Decimal('0.00'), output_field=DecimalField()))['total']) - self.purchase.down_payment
        context['comp'] = {
...
class PurchasePaymentDetailView(LoginRequiredMixin, ValidatePermissionRequiredMixin, DetailView):
    model = Purchase
    template_name = 'purchasepayment/detail.html'
    permission_required = 'view_purchase'
    url_redirect = reverse_lazy('hermes:purchasepayment_list')
    
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['title'] = 'Detalles de la Compra'
        context['entity'] = 'Compras'
        context['purchase_payment'] = PurchasePayment.objects.filter(purchase=self.object)
        context['purchase_payments'] = (PurchasePayment.objects.filter(purchase=self.object).aggregate(total=Coalesce(Sum('amount'), Decimal('0.00'), output_field=DecimalField()))['total']) + self.object.down_payment
